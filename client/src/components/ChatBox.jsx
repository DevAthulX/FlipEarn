...
const ChatBox = () => {
    ...
    const handleSentMessage = async (e) => {
        ...
        setMessages([...messages , {id: Date.now() , chatId: chatId , sender_id: user.id, message: newMessage, createdAt: new Date()}]);
        setNewMessage("")
    }
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-100 flex items-center justify-center sm:p-4   ">
            <div className="bg-white sm:rounded-lg shadow-2xl w-full max-w-2xl h-screen sm:h-[600px] flex flex-col">
                ...
                <div className="bg-gradient-to-r from-indigo-600 to-indigo-400 text-white p-4 sm:rounded-t-lg flex items-center justify-between">
                    ...
                    <button onClick={() => dispatch(clearChat())} className="ml-4 p-1 hover:bg-white/20 hover:bg-opacity-20 rounded-lg transiton-colors"><X className="w-5 h-5 " /></button>
                </div>
                {/* Messages Area */}
                ...
                {chat?.listing?.status === "active" ? (
                    <form onSubmit={handleSentMessage} className="p-4 bg-white border-t border-gray-200 rounded-b-lg">
                        <div className="flex items-end space-x-2">
                            ...
                            }} placeholder="Type your message..." className="flex-1 resize-none border-gray-300 rounded-lg px-4 py-2 focus:outline-indigo-500 max-h-32" rows={1} />
                            <button disabled={!newMessage.trim() || isSending} type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white p-2.5 rounded-lg disabled:opacity-50 transition-colors">{isLoading ? <Loader2Icon className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5 " />}
                            </button>
                        </div>
