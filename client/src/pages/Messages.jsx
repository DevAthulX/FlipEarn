...
import { setChat } from '../app/features/chatSlice' // Add your actual path
...
const Messages= () => {
  ...
  const formatTime =(dateStrlng) => {
            if(!dateStrlng) return;
            const date = parseISO(dateStrlng)
            if(isToday(date)) {
    ...
    <div className="mx-auto min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
         <div className="py-10">
          ...
          <div  className="relative max-w-xl mb-8">
           ...
           <input className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-indigo-500" onChange={(e) => setSearchQuery(e.target.value)} type="text" placeholder="Search conversation...." value={searchQuery}/>
          </div>
          {/* Chatlist */}
          {loading ? (
            <div className="text-center text-gray-500 py-20">Loading messages ...</div>
          ): filteredChats.length ===0 ? (
              <div className="bg-white rounded-lg shadow-xs border border-b-gray-200 p-16 text-center">
